...
export default class Utils {
  ...
  static getSunRiseUrl(
    latitude: number,
    longitude: number,
    dateStart?: Moment,
    dateEnd?: Moment
  ) {
    const baseUrl = process.env.SUNRISE_API_BASE_URL;
    return `${baseUrl}/json?lat=${latitude}&lng=${longitude}${
      dateStart ? `&date_start=${dateStart.format("YYYY-MM-DD")}` : ""
    }${dateEnd ? `&date_end=${dateEnd.format("YYYY-MM-DD")}` : ""}`;
  }
  ...
  static getTidePredictionsUrl(
    ...
    const baseUrl = process.env.TIDE_API_BASE_URL;
  ...
  static getStationUrl(stationId: string) {
    const baseUrl = process.env.TIDE_API_BASE_URL;
